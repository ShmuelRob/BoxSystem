...
namespace ManageSystemDll
{
    public class ManageSystem<T> where T : IContainable<T>
    {
        ...
        public LinkedList<T> ShowByTime(TimeSpan ts)
        {
        ...
        LinkedList<ProductData<T>> ShowProductsByTime(TimeSpan ts)
        {
            var list = new LinkedList<ProductData<T>>();
